...
# from models.threshold import DECNN
# from models.project import DECNN
...
class Instructor:
    def __init__(self, opt):
        ...
        if self.opt.baseline != 'none':
            self.trainset = ABSADataset(opt.dataset_file['train'], word_dict, opt, 'train', self.opt.baseline)
        else:
            self.trainset = ABSADataset(opt.dataset_file['train'], word_dict, opt, 'train')
        self.testset = ABSADataset(opt.dataset_file['test'],word_dict, opt, 'test')
        self.valset = ABSADataset(opt.dataset_file['dev'],word_dict, opt, 'dev')


        logger.info('>>> Dataset: {}, Size: {}, Use Prototypes: {}'.format(self.opt.dataset, self.opt.size, self.opt.lm))
        logger.info('>>> Train Samples: {}, After Duplication: {}.'.format(
            (len(self.trainset) // (3+1)) if self.opt.baseline != 'none' else len(self.trainset), len(self.trainset)))
        logger.info('>>> Test Samples: {}, Dev Samples: {}\n'.format(len(self.testset), len(self.valset)))
    ...
    def _train(self, criterion, optimizer, train_data_loader, test_data_loader, dev_data_loader, is_training):
        ...
        for epoch in range(self.opt.num_epoch):
            # weight_now = ((self.opt.num_epoch - 1) - epoch) / (self.opt.num_epoch - 1) * 0.2
            weight_now = 1.0
            weight_select = torch.from_numpy(np.array([1., weight_now])) # DECAY from 0.2
            # weight_select = torch.from_numpy(np.array([1., 0.2])) # FIX at 0.2
            ...
            for i_batch, sample_batched in enumerate(train_data_loader):
                ...
                optimizer.zero_grad()
                inputs = [sample_batched[col].to(self.opt.device) for col in ['x', 'mask', 'lmwords', 'lmprobs', 'data_type']]
                ...
                # proto_indicator = sample_batched['data_type'].to(self.opt.device) # b, 2
                # proto_weight = weight_select.to(self.opt.device).unsqueeze(0).repeat(proto_indicator.shape[0], 1)
                # weight_per_token = torch.sum(proto_weight * proto_indicator, dim=-1).unsqueeze(1).repeat(1, self.opt.max_sentence_len) # b
                #
                # aspect_loss_per_token =  torch.sum(-1 * torch.log(outputs) * aspect_y.float(), dim =-1)
                #
                # aspect_loss =  torch.sum(aspect_loss_per_token * weight_per_token) / (length + 1e-6)
            ...
            logger.info('Current Max Metrics Index : {} Current Min Loss Index : {} {} Tau : {:.2f} Prototype Weight: {:.2f}'
                        .format(dev_metric_list.index(max(dev_metric_list)), dev_loss_list.index(min(dev_loss_list)), epoch_time, tau_now, weight_now))
    ...
    def _evaluate_acc_f1(self, data_loader, epoch, tau_now):
        ...
        with torch.no_grad():
            for t_batch, t_sample_batched in enumerate(data_loader):
                inputs = [t_sample_batched[col].to(self.opt.device) for col in ['x', 'mask', 'lmwords', 'lmprobs', 'data_type']]
...
def main():
    ...
    parser.add_argument('--dataset', default='laptop', type=str, help='laptop restaurant')
    ...
    parser.add_argument('--lm', default='nolabel_prototype', type=str, help='language models: bert_base, bert_cross')
    parser.add_argument('--class_num', default=7, type=int, help='class number')
    parser.add_argument('--seed', default=123, type=int, help='random seed')
    parser.add_argument('--log_step', default=5, type=int)
    parser.add_argument('--size', default='SMALL', type=str, help='SMALL MEDIUM LARGE FULL')
    ...
    parser.add_argument('--name', default='NONE', type=str, help='NONE')
    parser.add_argument('--baseline', default='FINAL', type=str, help='none')
    # parser.add_argument('--pseudo_scale', default=1.0, type=float, help='NONE')
    ...
    dataset_files = {
        'restaurant': {
            'train': './data/restaurant/train/',
            'test': './data/restaurant/test/',
            'dev': './data/restaurant/dev/'
        },
        'laptop': {
            'train': './data/laptop/train/',
            'test': './data/laptop/test/',
            'dev': './data/laptop/dev/'
        },
        'device': {
            'train': './data/device/train/',
            'test': './data/device/test/',
            'dev': './data/device/dev/'
        },
        'service': {
            'train': './data/service/train/',
            'test': './data/service/test/',
            'dev': './data/service/dev/'
        }
    }
    input_colses = {
    ...
    log_file = './log/{}/{}-{}.log'.format(opt.dataset, opt.name, strftime("%y%m%d-%H%M%S", localtime()))
